...
private:
    ...
    bool helpRequested(int argc) {
        return argc <= 1 || Values.count("help") > 0;
    }
...
public:
    ...
    PhaseResult execute(TopologyManager& topology) const {
        ...
        std::cout << "TopologyDescription after Phase " << _phaseNum << ": "
...
private:
    void parseTest(fs::path testFilePath) {
        ...
        std::cout << std::endl << "Parsing " << testFilePath.string() << std::endl;
...
private:
    ...
    bool matchesFilter(const fs::directory_entry& entry, std::vector<std::string> filters) {
        ...
        for (const auto& filter : filters) {
            if (filePath.filename().string().find(filter) != std::string::npos) {
                return true;
            }
        }
